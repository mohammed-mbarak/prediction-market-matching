...
// Move OrderTable component OUTSIDE of OrderBook
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {orders.map((level, index) => (
          <div key={index} className="px-4 py-3 hover:bg-gray-50 flex justify-between items-center">
            <div className="flex items-center gap-3">
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${isBid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {level.price}¢
              </span>
...
const OrderBook = ({ orderBook, loading }) => {
  ...
  // Safely access the data with defaults
